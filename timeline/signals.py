...
class_skipping_student = Signal(providing_args=['instance'])  # skipping lessons (for students)
#
...
@receiver(class_skipping_student, dispatch_uid='notify_class_skipping_student')
def notify_class_skipping_student(sender, **kwargs):
    c = kwargs['instance']
    owl = Owl(
        template='mail/class/student/skipping.html',
        ctx={
            'c': c,
        },
        to=[c.customer.user.email],
